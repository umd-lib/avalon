...
RSpec.describe 'config/initializers/transcription.rb' do
  def reload_initializer!
    load Rails.root.join('config/initializers/transcription.rb')
  end
  ...
  context 'when transcription is disabled' do
    ...
    it 'does not raise, regardless of provider config' do
      expect { reload_initializer! }.not_to raise_error
    end
  end
  ...
  context 'when transcription is enabled' do
    ...
    context 'with an unknown default_provider' do
      ...
      it 'raises' do
        expect { reload_initializer! }.to raise_error(/Invalid Settings.transcription.default_provider/)
      end
    end
    ...
    context 'with aws_transcribe but no output_bucket configured' do
      ...
      it 'raises' do
        expect { reload_initializer! }.to raise_error(/output_bucket must be set/)
      end
    end
  end
end
