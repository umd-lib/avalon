...
class SpeedyAF::Proxy::Lease < SpeedyAF::Base
  ...
  def lease_is_active?
    return false if attrs['begin_time_dtsi'].blank? || attrs['end_time_dtsi'].blank?
    start_of_day(Date.today) >= attrs['begin_time_dtsi'] && end_of_day(Date.today) <= attrs['end_time_dtsi']
  end
end
