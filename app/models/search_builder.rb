...
class SearchBuilder < Blacklight::SearchBuilder
  ...
  def limit_to_inheritance_enabled_items(_permission_types = discovery_permissions, ability = current_ability)
    ...
    [policy_clauses(permission_types: [:edit]), "(*:* AND NOT disable_inheritance_bsi:true AND (#{(Array(read_policy_clauses) + read_access_clauses).join(" OR ")}))", "(disable_inheritance_bsi:true AND (#{(read_access_clauses + ["read_access_group_ssim:(#{user_visibility_groups.join(" OR ")})"] + discover_visibility_clauses).join(" OR ")}))"].compact.join(" OR ")
