...
class CatalogController < ApplicationController
  ...
  configure_blacklight do |config|
    ...
    # UMD Customization
    config.add_facet_field 'course_title_ssim', label: 'Course Name', limit: 5, if: Proc.new {|context, config, opts| context.current_ability.can?(:read, :administrative_facets)}, group: "workflow", solr_params: { 'facet.contains.ignoreCase' => true }
    # End UMD Customization
    config.add_facet_field 'date_digitized_ssim', label: 'Date Digitized', limit: 5, suggest: true, if: Proc.new {|context, config, opts| context.current_ability.can?(:read, :administrative_facets)}, group: "workflow"
