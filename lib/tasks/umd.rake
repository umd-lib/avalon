...
def move_dropbox_files_to_archive(archive_dir, dry_run=false)
  ...
  if num_migration_failed > 0
    ...
    perform_result.migrate_drop_box_files_result.failed_migrations.each do |result|
      puts "#{result.master_file_id},#{result.dropbox_file_location},#{result.archive_location},#{result.reason}"
    end
  end
  ...
  if num_deleted_failed > 0
    ...
    puts "---------"
    perform_result.delete_files_result.each do |result|
      puts "#{result.dropbox_file_location},#{result.reason}"
    end
  end
end
